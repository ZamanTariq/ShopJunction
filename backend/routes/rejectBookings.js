...
//Get All Bokings
let RejectBooking = require('../models/rejectBooking');
router.get('/', async (req, res) => {
	const bookings = await RejectBooking.find().sort('');
	res.send(bookings);
});
//Show User their booking
router.get('/:userId', async (req, res) => {
	const bookings = await RejectBooking.find({ userId: req.params.userId }).sort('bookingDate');
	res.send(bookings);
});
router.get('/merchant/:merchantId', async (req, res) => {
	const bookings = await RejectBooking.find({ merchantId: req.params.merchantId }).sort('bookingDate');
	res.send(bookings);
});
//Delete Booking By User
router.route('/delete/:id').get(function(req, res) {
	RejectBooking.findByIdAndRemove({ _id: req.params.id }, function(err, booking) {
		if (err) res.json(err);
	...
	booking
		...
		.then((booking) => {
            ...
            let mailOptions = {
                ...
                subject: "Booking Rejection Mail",
				html: "<h2>Your Booking Request has been Cancel  </h2>" +
