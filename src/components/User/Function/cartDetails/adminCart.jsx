...
export default class AdminCart extends Component {
...
handleDelete=cart=> {
      ...
      axios.get('http://localhost:5000/adminaddtocart/delete/'+cart._id)
...
updateInputsArray(index, newValue) {
     ...
     updatedArray[index].totalPrice=newValue*(updatedArray[index].price-(updatedArray[index].price*(updatedArray[index].discount/100)))
     this.setState({
          cart: updatedArray,
      });
  }
...
axios.post('http://localhost:5000/adminproduct/updateStock/'+cart.productId,obj)
.then(res => console.log(res.data),orderPlacementMessage(),this.handleDelete(cart))
.catch(err => console.log(err))

axios.post('http://localhost:5000/newuser/resetdiscount/'+ this.getUserId())
.then(res => console.log(res.data))
.catch(err => console.log(err))      
    } catch (error) {
        console.log(error)
...
handleUpdate=carts=> {
      axios.post('http://localhost:5000/adminaddtocart/update/'+carts._id,carts)
          ...
          {this.state.cart.map((carts,index) => (
            ...
            <tr key={carts._id}  >
              ...
              <span className="badge badge-warning  m-2 text-muted"><h6>{carts.quantity}</h6></span>
                <button key={carts} disabled={(carts.discount>0&&carts.quantity===3) || carts.quantity===12}  onClick={e =>this.handleIncrement(index,carts.quantity+1)}  className="btn btn-primary btn-sm">+</button>
             </td>
             
              <td>{carts.quantity===1?carts.price-(carts.price*(carts.discount/100)):carts.totalPrice}</td>
              <td>
  ...
  <div className="card-body card-body-cascade text-center pb-0">
    ...
    <div className="card-footer text-muted text-center mt-4">
      Thanks For Shopping !
          ...
          <td> {carts.stock}</td>
          <td> {carts.discount>0? carts.discount :null}</td>
            </tr>
          ))}
        </tbody>
