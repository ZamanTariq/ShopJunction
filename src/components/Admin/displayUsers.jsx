...
handleDelete=users=> {
  const product = this.state.users.filter(m => m._id !== users._id);
  this.setState({ users:product });
  axios.get('http://localhost:5000/newuser/delete/'+users._id)
  ...
  .catch(err => console.log(err))
    ...
    render() {
      const data = {
        ...
        rows: this.state.users.map((users) => {
          return {
            firstname: users.firstname,
            lastname: users.lastname,
            email: users.email,
            phone: users.phone,
          active:<div>{users.activeUser?<small>Y</small>:<small>N</small>}</div>,
            address: users.address,
            action: (
              <React.Fragment>
                <Link
                  className="ml-5 text-primary"
                  to={"/edit-users-detail/" + users._id}
                  title="Edit"
                >
    ...
    <div className="panel panel-default my-auto" id="transaction">
      <div className="panel-heading main-color-bg">
        <h3 className="panel-title text-center "><b>OUR CUSTOMERS</b></h3>
      </div>
